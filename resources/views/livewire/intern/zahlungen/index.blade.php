                ...
                <x-custom.table.table>
                    ...
                    <x-slot:tbody>
                        <x-custom.table.responsive.tr>
                            <x-custom.table.responsive.td colspan="3" class="!p-2 font-medium cursor-pointer !whitespace-normal text-right" :text="'Gesamtbetrag: '. $zahlungGesamt"/>
                        ...
                        @foreach(json_decode($zahlungen, false) as $month => $zahlung)
                            ...
                            @foreach($zahlung as $key => $item)
                                @if($item->betrag >= 25.00)
                                <x-custom.table.responsive.tr>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-success-500 dark:text-success-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal align-top" wire:click="show('{{ $item->id }}')" :text="$key + 1"/>
                                ...
                                @elseif($item->betrag <= 24.99)
                                <x-custom.table.responsive.tr>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-blue-500 dark:text-blue-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal align-top" wire:click="show('{{ $item->id }}')" :text="$key + 1"/>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-blue-500 dark:text-blue-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal align-top" wire:click="show('{{ $item->id }}')" :text="$item->name"/>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-blue-500 dark:text-blue-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal align-top text-right" wire:click="show('{{ $item->id }}')" :text="number_format($item->betrag, 2, ',', '.').' €'"/>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-blue-500 dark:text-blue-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal align-top text-center" wire:click="show('{{ $item->id }}')" :text="$item->date_of_payment ? Carbon::parse($item->date_of_payment)->isoFormat('DD.MM.YY') : null"/>
                                    <x-custom.table.responsive.td class="!p-2 font-medium {{ $item->bezahlt ? 'text-blue-500 dark:text-blue-700' : 'text-red-500 dark:text-red-700' }} cursor-pointer !whitespace-normal text-center">
                                        <div class="flex justify-end items-center leading-none">
                                            @if($item->bezahlt)
                                                <x-custom.button.button-blank wire:click="pay('{{ $item->id }}', '0')" color="red-2" wire:loading.remove>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-x-circle h-4 w-4" viewBox="0 0 16 16">
